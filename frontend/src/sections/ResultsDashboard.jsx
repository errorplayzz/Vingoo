  ...
  return (
    <AnimatePresence>
      ...
      >
        ...
        <div className="px-6 pt-6 pb-4 border-b border-slate-200 flex items-start justify-between gap-4 flex-shrink-0">
          <div>
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Request Review</p>
            <p className="font-mono font-bold text-slate-900 text-[15px]">{account.account_id}</p>
            <div className="flex flex-wrap gap-1.5 mt-2">
              <ScorePill score={account.suspicion_score} />
              {account.ring_id && <RingTag ringId={account.ring_id} />}
        ...
        {done ? (
          ...
          <div className="flex-1 flex flex-col items-center justify-center px-6 py-10 text-center">
            ...
            <p className="text-[15px] font-bold text-slate-900 mb-1">Request Submitted</p>
            <p className="text-[12.5px] text-slate-500 mb-4">Your review request has been logged and will be processed.</p>
            {done.review_id && (
              <p className="text-[11px] text-slate-400 font-mono mb-1">Ref: {done.review_id}</p>
            )}
            {done.review_deadline && (
              <p className="text-[11px] text-slate-400">Review deadline: {done.review_deadline}</p>
            )}
            <button
              onClick={onClose}
              className="mt-6 text-[13px] font-semibold px-5 py-2.5 rounded-xl bg-indigo-600 text-white hover:bg-indigo-700 transition-colors"
            >
              Close
            </button>
          </div>
          ...
          <form onSubmit={handleSubmit} className="flex-1 flex flex-col px-6 py-5 gap-4 overflow-y-auto">
            ...
            <div className="flex flex-col gap-1">
              ...
              <input
                ...
                className="w-full text-[12.5px] px-3 py-2 rounded-lg border border-slate-200 bg-slate-50
                           text-slate-900 placeholder:text-slate-400 outline-none focus:border-indigo-400 focus:bg-white transition-colors"
              />
            </div>
